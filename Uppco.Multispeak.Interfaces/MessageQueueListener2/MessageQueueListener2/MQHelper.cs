...
namespace Uppco.Multispeak.Interfaces
{
    public class MQHelper : IDisposable
    {
        ...
        public MQConnection _activeConnection; 
        ...
        public MQHelper()
        {
            ...
            FileInfo fi = new FileInfo(Path.Combine(System.Environment.CurrentDirectory, "bin\\MQConfig.xml"));
            if (!fi.Exists)
                fi = new FileInfo(Path.Combine(System.Environment.CurrentDirectory, "MQConfig.xml"));

            _settings = (Settings)SettingsManager.GetSettings(fi.FullName, typeof(Settings));
            //FileInfo fi = new FileInfo(Path.Combine(HostingEnvironment.ApplicationPhysicalPath, "bin\\MQConfig.xml"));
            //if (!fi.Exists)
            //    fi = new FileInfo(Path.Combine(HostingEnvironment.ApplicationPhysicalPath, "MQConfig.xml"));
